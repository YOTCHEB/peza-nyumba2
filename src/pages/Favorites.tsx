  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{favorites.length} saved</p>
        {favorites.length === 0 ? (
          ...
          <div className="mt-4 grid gap-4 grid-cols-2 sm:grid-cols-3">{favorites.map(f => <ListingCard key={f.listing.id} listing={f.listing} />)}</div>
        )}
        </div>
      </div>
      </main>
