    ...
    <header className="sticky top-0 z-50 border-b glass">
      <div className="container flex h-16 items-center justify-between">
        ...
        <nav className="hidden items-center gap-0.5 sm:flex">
          ...
          {isAuthenticated && (
            <Link to="/favorites" className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition-all ${isActive("/favorites") ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-secondary"}`}>
              <HiOutlineHeart size={18} /> Saved
            </Link>
          )}
          {user?.role === "landlord" && (
            <Link to="/dashboard" className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition-all ${isActive("/dashboard") ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-secondary"}`}>
              <HiOutlineUser size={18} /> Dashboard
            </Link>
          )}
          {user?.role === "admin" && (
            <Link to="/admin" className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition-all ${isActive("/admin") ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-secondary"}`}>
              <HiOutlineShieldCheck size={18} /> Admin
            </Link>
          )}
          <div className="ml-2 h-6 w-px bg-border" />
          {isAuthenticated ? (
            <button onClick={logout} className="ml-2 flex items-center gap-1.5 rounded-lg border border-border px-3 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-secondary transition-all">
              <HiOutlineLogout size={18} /> Logout
            </button>
          ) : (
            <Link to="/login" className="ml-2 flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground transition-all hover:shadow-lg hover:shadow-primary/20">
              <HiOutlineLogin size={18} /> Login
            </Link>
          )}
        </nav>

        <button onClick={() => setOpen(!open)} className="flex h-12 w-12 items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary sm:hidden transition-all touch-manipulation" aria-label="Toggle menu">
      ...
      <div className={`fixed right-0 top-0 z-50 h-full w-[280px] border-l shadow-xl sm:hidden bg-card transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "translate-x-full"}`}>
